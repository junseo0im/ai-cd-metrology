...
def recover_native_pixels(
    ...
    """Remove an explicitly declared nearest-neighbor repetition factor."""
    ...
    paired_height = (image.shape[0] // 2) * 2
    paired_width = (image.shape[1] // 2) * 2
    row_pairs = image[:paired_height].reshape(
        paired_height // 2, 2, *image.shape[1:]
    )
    column_source = image[:, :paired_width]
    column_pairs = column_source.reshape(
        image.shape[0], paired_width // 2, 2, *image.shape[2:]
    )
    row_consistency = float(
        np.mean(np.all(row_pairs[:, 0] == row_pairs[:, 1], axis=tuple(range(1, row_pairs[:, 0].ndim))))
    )
    column_consistency = float(
        np.mean(
            np.all(
                column_pairs[:, :, 0] == column_pairs[:, :, 1],
                axis=tuple(range(2, column_pairs[:, :, 0].ndim)),
            )
        )
    )
...
def compare_image(
    ...
    *,
    ...
    for y_fraction, (native_y, candidates, diagnostics) in scanlines.items():
        ...
        message = (
            f"native_y_px={native_y}; duplicate_factor={duplicate_factor}; "
...
def main() -> None:
    ...
    parser.add_argument("--duplicate-factor", type=int, choices=(1, 2), default=1)
    parser.add_argument(
        "--output-root",
        type=Path,

...
def test_recover_native_pixels_auto_detects_2x_repetition() -> None:
    native = np.arange(3 * 4, dtype=np.uint8).reshape(3, 4)
    enlarged = np.repeat(np.repeat(native, 2, axis=0), 2, axis=1)

    recovery = recover_native_pixels(enlarged)

    np.testing.assert_array_equal(recovery.image, native)
    assert recovery.duplicate_factor == 2


def test_recover_native_pixels_detects_shifted_2x_repetition() -> None:
    native = np.arange(5 * 7, dtype=np.uint8).reshape(5, 7)
    enlarged = np.repeat(np.repeat(native, 2, axis=0), 2, axis=1)
    shifted = np.pad(enlarged, ((0, 0), (1, 0)), mode="edge")

    recovery = recover_native_pixels(shifted)

    assert recovery.duplicate_factor == 2
    assert recovery.offset_x == 1
    np.testing.assert_array_equal(recovery.image[:, : native.shape[1]], native)
